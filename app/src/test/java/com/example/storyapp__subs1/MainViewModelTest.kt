...
class MainViewModelTest {
    ...
    @Mock
    private lateinit var userStoryRepository: UserStoryRepo

    private val token = "token"

    @Test
    fun `when Get Paging Stories Should Not Be Null and Return Data`() = runTest {
        val dummyStories = DummyData.generateDummyNewsEntity()
        val data: PagingData<ListStoryItem> = QuotePagingSource.snapshot(dummyStories)
        val expectedStories = MutableLiveData<PagingData<ListStoryItem>>()
        expectedStories.value = data
        Mockito.`when`(userStoryRepository.getPagingStories(token)).thenReturn(expectedStories)

        val mainViewModel = MainViewModel(userStoryRepository)
        val actualStories: PagingData<ListStoryItem> = mainViewModel.getPagingStories(token).getOrAwaitValue()
